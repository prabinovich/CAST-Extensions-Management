...
func writeCentralHeader (f *os.File, s string, dbHost string, dbPort string) () {
	...
	f.WriteString("   <PackName >ADG_FULL_CENTRAL</PackName>\n")
 	f.WriteString("\n")  
...
func writeLocalHeader (f *os.File, s string, dbHost string, dbPort string) () {
	...
	f.WriteString(fmt.Sprintf(" <ServerInstall ProfileSystem=\"PROFILE_NAME\" ServerType=\"CASTStorageService\" UserSystem=\"%s\" SystemPassword=\"%s\" ServerName=\"%s:%s\" >\n", DB_USER, DB_PASSWORD, dbHost, dbPort))
	f.WriteString(fmt.Sprintf("  <RefreshDatabase DbName=\"%s\" >\n", s))
	f.WriteString("   <PackName >BASE_LOCAL</PackName>\n")
	f.WriteString("   <PackName >ADG_LOCAL_APPW</PackName>\n")
	f.WriteString("   <PackName >KMS_LOCAL</PackName>\n")
	f.WriteString("   <PackName >KMS2_LOCAL</PackName>\n")
	f.WriteString("\n")
	f.WriteString("	<!-- Extensions: install most recent that has been downloaded -->\n")
...
func writeMngtHeader (f *os.File, s string, dbHost string, dbPort string) () {
...
func main() {
	...
	if (len(os.Args) != 5) {
		...
		fmt.Println("Example: upgradeSchemaExtensions.exe \"C:\\Program Files\\Cast\\8.3.3\" localhost 2280 foo%")
		os.Exit(1)
	}
	...
	qry := fmt.Sprintf("select schema_name from information_schema.schemata where schema_name like '%s_mngt' or schema_name like '%s_central' or schema_name like '%s_central' order by schema_name", sPrefix, sPrefix, sPrefix)
